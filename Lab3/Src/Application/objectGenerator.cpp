...
GeometricObjectGenerator::GeometricObjectGenerator(LPDIRECT3DDEVICE9 pDevice, int squaresM, int squaresN) : M(squaresM), N(squaresN)
{
  ...
  vertNum = (M + 1) * (N + 1);
  triNum = M * N * 2;
  Plane T(D3DXVECTOR3(50.0f,0.0f,0.0f),D3DXVECTOR3(0.0f,50.0f,0.0f));
...
DWORD GeometricObjectGenerator::colorFromPoint(D3DXVECTOR3 point)
{
  ...
  return D3DCOLOR_COLORVALUE(norm.x, norm.y, norm.z,1.0f);
}
...
void GeometricObjectGenerator::LoadTexture(char *filename, char **mipmaps, int number)
{
...
DWORD *GeometricObjectGenerator::TexMin()
{
  return &minfilter;
}

DWORD *GeometricObjectGenerator::TexMag()
{
  return &magfilter;
}

DWORD *GeometricObjectGenerator::TexMip()
{
  return &mipfilter;
}

float *GeometricObjectGenerator::TexBias()
{
  return &mipmapbias;
}
...
void MeshObjectGenerator::LoadTexture(char *filename, char **mipmaps, int number)
{
  texture = new LPDIRECT3DTEXTURE9;
  size_t size = strlen(filename) + 1;
  wchar_t *wFileName = new wchar_t[size];
...
DWORD *MeshObjectGenerator::TexMin()
{
  return &minfilter;
}

DWORD *MeshObjectGenerator::TexMag()
{
  return &magfilter;
}

DWORD *MeshObjectGenerator::TexMip()
{
  return &mipfilter;
}

float *MeshObjectGenerator::TexBias()
{
  return &mipmapbias;
}